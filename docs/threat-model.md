...
- **Shipped now** (#111, #112, #114, landed after this document was first written): a strict
  Content-Security-Policy on both origins, built rather than hand-written. The landing
  (`landing/_headers`) runs `default-src 'self'`, `base-uri 'none'`, `object-src 'none'`,
  `form-action 'none'`, `frame-ancestors 'none'`, `connect-src 'none'`, and a `script-src`
  that permits exactly one generated inline hash — no `'unsafe-inline'`, no host source, so
  no external script can execute there at all. The app origin gets an equivalent policy
  generated by `scripts/build-pages.mjs`. Alongside it: Subresource Integrity and import-map
  integrity on the built app, a published reproducible-build hash manifest
  (`landing/integrity-manifest.json`, naming the commit it was built from), a build stamp in
  the shipped HTML, and a verify page (`landing/verify.html`) explaining how to check the
  hashes yourself. **All of this is on `develop` and reaches production at the next
  promotion to `main`** — until then wymber.app still serves the older baseline headers, so
  read this bullet as "shipped in the codebase", not "live for users right now".
- **Not shipped:** `Integrity-Policy` is **enforced on the landing but report-only on the
  app**, because `mindmap.js` lazy-loads Cytoscape through a classic `<script>` that does not
  yet carry an `integrity` attribute; enforcing today would break the map. `style-src` still
  allows `'unsafe-inline'`, since both surfaces use inline `style=` attributes — script
  injection is the threat that matters here, but this is a real remaining gap and not a
  rounding error. There is still no out-of-band attestation (a Code-Verify-style browser
  extension, tracked in #115), which is the only thing that would detect a host serving
  different code to different people.

Net: a user can now independently confirm that the code they are running matches the
published source, and the browser itself enforces most of the substitution surface. What
remains unclosed is the honest core of the problem, and it does not go away with more
headers: **a client cannot attest to its own integrity.** Every check described above is
performed by, or delivered from, the thing being checked. That is why ADR-0003 treats
out-of-band verification as the only real anchor, and why this section will still exist
after #115 ships.
