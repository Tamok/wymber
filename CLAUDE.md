...
- **DB path is relative and hardcoded.** `database.py` uses `sqlite:///./data/traumappd.db` (relative to the current working directory) and does **not** read `env_config.DATABASE_PATH` (that setting is effectively dead). Always launch from the repo root so it resolves to `./data/`.
